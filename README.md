...
### Создание исходных файлов (эти примеры есть в папке scripts)
...
Port:             5432
```
6) По умолчанию используется подключение с именем `postgres_default`.
7) После клонирования очищаем папку **dags**.
8) Запускаем генератор в контейнере `Airflow`:</br>
